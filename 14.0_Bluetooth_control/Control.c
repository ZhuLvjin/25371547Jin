/*
 * 14.0_Bluetooth_control: 蓝牙遥控小车 (任务26 综合系统实验)
 *
 * 架构: 手机蓝牙调试器APP →(经典蓝牙SPP)→ JDY-16蓝牙模块 →(UART1 9600)→ Hi3861
 *       →(UART2 115200, 0xFC协议帧)→ STM32 → 电机
 *
 * 按键定义(蓝牙调试器发送字符):
 *   W=前进  S=后退  A=左转  D=右转  O=停止
 *   I=直行(100,100)  K=直行(150,150)
 *   Z=学生任务示例: 一次按键完成一套系列动作
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
/***通信协议***/
/* 发送给stm32的数据协议
 * 参数: 左右电机的速度 (rad/s的100倍, 例如: 设置转速为1rad/s则传入100)
 */
...
/***** 学生任务示例: 一次按键(Z)完成一系列动作 *****/
static void car_sequence(void)
{
    car_forward();      hi_sleep(2000);   /* 前进2秒 */
    car_stop();         hi_sleep(500);    /* 停0.5秒 */
    car_left();         hi_sleep(2000);   /* 左转2秒 */
    car_stop();         hi_sleep(500);
    car_backward();     hi_sleep(1500);   /* 后退1.5秒 */
    car_stop();         hi_sleep(500);
    ...
    while(1)
    {
        ...
        if(bluetooth_flag[0]!=0)
        {
            switch(bluetooth_flag[0])   //判断接收的字符
            {
                ...
                case 'I': stm32motor_control(100, 100); break;
                case 'K': stm32motor_control(150, 150); break;
                case 'Z': car_sequence(); break;   /* 学生任务: 一键系列动作 */
                default:  break;
            }
