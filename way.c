...
static unsigned int MotorUartInit(void)
{
    ...
    WifiIotUartExtraAttr extra;

    (void)memset(&extra, 0, sizeof(extra));
    extra.txFifoLine = WIFI_IOT_FIFO_LINE_ONE_EIGHT;
    extra.rxFifoLine = WIFI_IOT_FIFO_LINE_ONE_EIGHT;
    extra.flowFifoLine = WIFI_IOT_FIFO_LINE_ONE_EIGHT;
    extra.txBlock = WIFI_IOT_UART_BLOCK_STATE_NONE_BLOCK;
    extra.rxBlock = WIFI_IOT_UART_BLOCK_STATE_NONE_BLOCK;
    extra.txBufSize = 256;
    extra.rxBufSize = 256;
    extra.txUseDma = WIFI_IOT_UART_NONE_DMA;
    extra.rxUseDma = WIFI_IOT_UART_NONE_DMA;

    IoSetFunc(WIFI_IOT_IO_NAME_GPIO_11, WIFI_IOT_IO_FUNC_GPIO_11_UART2_TXD);
    IoSetFunc(WIFI_IOT_IO_NAME_GPIO_12, WIFI_IOT_IO_FUNC_GPIO_12_UART2_RXD);
    return UartInit(MOTOR_UART, &uartAttr, &extra);
}
