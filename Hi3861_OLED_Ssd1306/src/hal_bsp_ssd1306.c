...
/* Display one 16x16 Chinese char, idx 0~4: HongMengXianFengHao */
void SSD1306_ShowChineseByIndex(uint8_t x, uint8_t y, uint8_t idx)
{
    uint8_t i;
    SSD1306_SetPos(x, y);
    for (i = 0; i < 16; i++) {
        SSD1306_WiteData(GB16_HongMeng[idx][i]);
    }
    SSD1306_SetPos(x, y + 1);
    for (i = 0; i < 16; i++) {
        SSD1306_WiteData(GB16_HongMeng[idx][i + 16]);
    }
}
