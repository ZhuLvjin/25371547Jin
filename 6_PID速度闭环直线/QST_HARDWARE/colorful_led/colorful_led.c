...
/*****前后12颗灯一起跑马灯*****/
void L_runingled(void)    //前灯+尾灯同时跑马灯
{
    u8 i, j;

    while(1)   // 循环跑马灯
    {
        /* 正向流光：亮点从第 1 盏走到第 6 盏（前后同步） */
        for(j=1; j<=6; j++)
        {
            for(i=1; i<=6; i++)    //把每颗灯的颜色写入数组
            {
                if(i == j)
                {
                    L_ws2812_rgb(i, WS_WHITE);   //前灯当前这盏亮
                    R_ws2812_rgb(i, WS_WHITE);   //尾灯当前这盏亮
                }
                else
                {
                    L_ws2812_rgb(i, WS_DARK);    //前灯其余熄灭
                    R_ws2812_rgb(i, WS_DARK);    //尾灯其余熄灭
                }
            }
            L_ws2812_refresh(led_num);   //刷新前灯
            R_ws2812_refresh(led_num);   //刷新尾灯
            delay_ms(100);
        }

        /* 反向流光：从第 6 盏走回第 1 盏（前后同步） */
        for(j=6; j>=1; j--)
        {
            for(i=1; i<=6; i++)
            {
